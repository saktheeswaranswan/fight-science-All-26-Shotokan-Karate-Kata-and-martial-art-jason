...
// replaced emoji usage with this image
let headImg = null;
const crownEmoji = "👑";
...
function preload() {
  poseData = loadJSON('chikirrichikirii.json');
  mainVideo = createVideo(['cyberphysical.mp4']);
  video2 = createVideo(['cyberphysical.mp4']);

  // load your image (replace filename if different)
  headImg = loadImage('unnamed.jpg');
}
...
/*
  drawPoseOverlay — using headImg instead of emoji:
  - compute screen coordinates for each keypoint using scaling + shear (same as before)
  - draw skeleton & small dots using those screen coords (so perfectly aligned)
  - draw headImg at same screen coords (NOT sheared) using imageMode(CENTER)
  - shadow created by drawing image slightly offset and with tint/alpha via push/pop
*/
function drawPoseOverlay() {
  ...
  // --- Draw head images at the computed head screen positions (NOT sheared) ---
  imageMode(CENTER);
  let newLastHeads = [];
  ...
  personsScreen.forEach((ps, idx) => {
    ...
    // Draw a soft shadow by drawing the image slightly offset and semi-transparent, then the image
    if (headImg) {
      push();
      tint(0, 120); // semi-transparent dark shadow
      image(headImg, h.x + 6, h.y + 6, drawSize, drawSize);
      pop();

      // draw main image (no tint)
      image(headImg, h.x, h.y, drawSize, drawSize);
    } else {
      // fallback: draw a circle if image failed to load
      noStroke();
      fill(50);
      circle(h.x, h.y, drawSize);
    }
  });
  ...
  // restore imageMode default if you rely on text being default later
  imageMode(CORNER);
}
